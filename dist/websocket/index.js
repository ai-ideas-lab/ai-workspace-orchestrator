...
function setupWebSocket(server, config) {
    try {
        ...
        wss.on('connection', (ws) => {
            logger_js_1.logger.info('WebSocket client connected');
            connections.add(ws);
            ...
            ws.on('message', (message) => {
                try {
                    ...
                    logger_js_1.logger.debug('Received WebSocket message:', data);
                }
                catch (error) {
                    logger_js_1.logger.error('Error parsing WebSocket message:', error);
            ...
            ws.on('close', () => {
                logger_js_1.logger.info('WebSocket client disconnected');
        ...
        logger_js_1.logger.info(`WebSocket server started on port ${server.address()?.port}/ws`);
        return { wss, broadcast };
    }
    catch (error) {
        logger_js_1.logger.error('WebSocket setup failed:', error);
        throw error;
    }
}
