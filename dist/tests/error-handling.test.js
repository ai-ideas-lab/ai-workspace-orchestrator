...
const database_error_handler_js_1 = require("../utils/database-error-handler.js");
...
describe('Error Handling Integration Tests', () => {
    describe('Global Error Handler', () => {
        test('should handle synchronous errors', async () => {
            ...
            server_js_1.app.use('/test', router);
    ...
    describe('Custom Error Classes', () => {
        test('should handle ValidationError', async () => {
            ...
            router.get('/validation-error', (req, res) => {
                throw new errors_js_1.ValidationError('验证失败', 'email', { provided: 'invalid-email' });
            });
            server_js_1.app.use('/test', router);
            const response = await (0, supertest_1.default)(server_js_1.app)
        ...
        test('should handle DatabaseError', async () => {
            ...
            router.get('/database-error', (req, res) => {
                throw new errors_js_1.DatabaseError('数据库连接失败', { table: 'users' });
            });
            server_js_1.app.use('/test', router);
            const response = await (0, supertest_1.default)(server_js_1.app)
        ...
        test('should handle SystemError', async () => {
            ...
            router.get('/system-error', (req, res) => {
                throw new errors_js_1.SystemError('系统服务不可用', 'external-api');
            });
            server_js_1.app.use('/test', router);
            const response = await (0, supertest_1.default)(server_js_1.app)
    ...
    describe('Database Error Handler', () => {
        test('should handle Prisma unique constraint violation', () => {
            ...
            const error = database_error_handler_js_1.DatabaseErrorHandler.handlePrismaError(mockError, context);
            expect(error).toBeInstanceOf(errors_js_1.ValidationError);
        ...
        test('should handle Prisma record not found', () => {
            ...
            const error = database_error_handler_js_1.DatabaseErrorHandler.handlePrismaError(mockError, context);
            expect(error).toBeInstanceOf(errors_js_1.AppError);
        ...
        test('should handle unknown database errors', () => {
            ...
            const error = database_error_handler_js_1.DatabaseErrorHandler.handlePrismaError(mockError, context);
            expect(error).toBeInstanceOf(errors_js_1.DatabaseError);
    ...
    describe('Async Error Handler', () => {
        test('should handle async operation with retry', async () => {
            ...
            const errorHandler = new async_error_handler_js_1.AsyncErrorHandler();
            const result = await errorHandler.executeWithRetry(mockAsyncOperation, context, {
                maxRetries: 5,
        ...
        test('should handle async operation timeout', async () => {
            ...
            const errorHandler = new async_error_handler_js_1.AsyncErrorHandler();
            await expect(errorHandler.executeWithTimeout(slowOperation, 100, context)).rejects.toThrow('操作超时');
        });
        test('should handle async operation partial failure', async () => {
            ...
            const errorHandler = new async_error_handler_js_1.AsyncErrorHandler();
            const result = await errorHandler.executeBatchWithPartialFailure(operations, context);
    ...
    describe('Error Logging', () => {
        test('should log errors with proper context', async () => {
            let loggedErrors = [];
            const originalError = logger_js_1.logger.error;
            logger_js_1.logger.error = (...args) => {
            ...
            try {
                ...
                logger_js_1.logger.error = originalError;
            }
        });
    ...
    describe('Performance', () => {
        test('should handle concurrent error requests without performance degradation', async () => {
            ...
            server_js_1.app.use('/test', router);
            const requests = Array(concurrentRequests).fill(null).map(() => (0, supertest_1.default)(server_js_1.app).get('/test/concurrent-error').expect(500));
