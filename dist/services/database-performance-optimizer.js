...
exports.DatabasePerformanceOptimizer = void 0;
exports.analyzeDatabasePerformance = analyzeDatabasePerformance;
exports.generateDatabaseOptimizationReport = generateDatabaseOptimizationReport;
...
class DatabasePerformanceOptimizer {
    ...
    async recordQueryPerformance(query, executionTime, parameters, errorMessage) {
        ...
        if (queryPerformance.slow) {
            logger_js_1.logger.warn(`Slow query detected (${executionTime}ms): ${queryPerformance.query}`);
    ...
    analyzeQueryOptimization(queryPerformance) {
        for (const rule of this.optimizationRules) {
            if (rule.check(queryPerformance.query, queryPerformance.executionTime)) {
                ...
                logger_js_1.logger.info(`Query optimization suggestion: ${rule.recommendation}`);
            }
        }
    }
    ...
    clearQueryHistory() {
        ...
        logger_js_1.logger.info('Query history cleared');
    }
    setOptimizationEnabled(enabled) {
        this.isOptimizationEnabled = enabled;
        logger_js_1.logger.info(`Database optimization ${enabled ? 'enabled' : 'disabled'}`);
    }
    ...
    loadPerformanceMetrics() {
        logger_js_1.logger.debug('Loaded performance metrics');
    }
}
exports.DatabasePerformanceOptimizer = DatabasePerformanceOptimizer;
DatabasePerformanceOptimizer.instance = null;
...
function generateDatabaseOptimizationReport() {
    const optimizer = DatabasePerformanceOptimizer.getInstance();
    return optimizer.generateOptimizationReport();
}
async function executeMonitoredQuery(query, parameters, options) {
    const optimizer = DatabasePerformanceOptimizer.getInstance();
