...
describe('WorkflowBuilder', () => {
    ...
    test('应该能够编辑工作流信息', () => {
        ...
        expect(screen.getByText('修改后的工作流')).toBeInTheDocument();
    });
    test('应该能够删除工作流', () => {
        render(<WorkflowBuilder />);
        const createButton = screen.getByText('创建新工作流');
...
describe('AITaskNode', () => {
    ...
    test('应该能够编辑提示词', () => {
        const onConfigChange = jest.fn();
        render(<AITaskNode config={{}} onConfigChange={onConfigChange}/>);
        const textarea = screen.getByPlaceholderText(/提示词/i);
    ...
    test('应该能够预填充配置', () => {
        ...
        expect(screen.getByDisplayValue('初始提示词')).toBeInTheDocument();
    });
});
