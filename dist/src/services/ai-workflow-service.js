...
import { z } from 'zod';
import { aiWorkflowExecutor } from './ai-workflow-executor.js';
...
const createWorkflowSchema = z.object({
    ...
    steps: z.array(z.object({
        ...
        type: z.enum(['ai-task', 'condition', 'parallel', 'sequential']),
...
router.get('/workflows', (req, res) => {
    try {
        aiWorkflowExecutor.createSampleWorkflows();
        const workflows = [
            {
...
router.get('/workflows/:workflowId', (req, res) => {
    try {
        ...
        const workflows = aiWorkflowExecutor.createSampleWorkflows();
        const workflow = {
            id: workflowId,
            name: '示例工作流',
...
router.post('/workflows', (req, res) => {
    try {
        ...
        aiWorkflowExecutor.registerWorkflow(workflow);
        res.status(201).json({
            success: true,
            data: workflow,
            message: '工作流创建成功'
...
router.post('/workflows/:workflowId/execute', async (req, res) => {
    try {
        ...
        const execution = await aiWorkflowExecutor.executeWorkflow(workflowId, inputData);
        res.status(201).json({
            success: true,
            data: execution,
            message: '工作流执行已启动'
...
router.get('/workflows/:workflowId/executions', (req, res) => {
    try {
        const { workflowId } = req.params;
        const executions = aiWorkflowExecutor.getWorkflowExecutions(workflowId);
        res.json({
            success: true,
            data: executions,
...
router.get('/executions/:executionId', (req, res) => {
    try {
        ...
        const execution = aiWorkflowExecutor.getExecutionHistory(executionId);
        if (!execution) {
            return res.status(404).json({
                success: false,
                error: '执行记录不存在'
...
router.delete('/executions/:executionId', (req, res) => {
    try {
        const { executionId } = req.params;
        const cancelled = aiWorkflowExecutor.cancelExecution(executionId);
        if (!cancelled) {
            return res.status(400).json({
                success: false,
...
router.get('/executions', (req, res) => {
    try {
        const executions = aiWorkflowExecutor.getAllExecutions();
        res.json({
            success: true,
            data: executions,
            count: executions.length
...
router.post('/executions/cleanup', (req, res) => {
    try {
        ...
        const cleanedCount = aiWorkflowExecutor.cleanupExecutions(maxAge);
        res.json({
            success: true,
