...
export class AIEngineManager {
    ...
    isEngineAvailable(engine) {
        return this.activeEngines.has(engine.id) && engine.load <= 0.8;
    }
    getSpecifiedEngine(step) {
        if (!step.engineId)
            return null;
        const engine = this.engines.get(step.engineId);
        if (engine && this.isEngineAvailable(engine)) {
            return engine;
        }
    ...
    selectBestEngine(step) {
        ...
        const specifiedEngine = this.getSpecifiedEngine(step);
        if (specifiedEngine) {
            return specifiedEngine;
        }
        const suitableEngines = Array.from(this.engines.values()).filter(engine => this.isEngineAvailable(engine) && this.isEngineSuitable(engine, step));
    ...
    async getSystemStatus() {
        try {
            const dbStats = await databaseManager.getWorkflowStats();
    ...
    async executeWithFallbackEngine(engine, step) {
        ...
        try {
            ...
            this.updateEngineStats(engine.id, 0, false);
            console.error(`❌ 故障转移引擎执行失败: ${engine.name}`, {
                stepId: step.id,
                error: error instanceof Error ? error.message : String(error),
                engineId: engine.id
            });
            throw new Error(`故障转移引擎执行失败: ${engine.name} - ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    async executeOpenAIStep(engine, step) {
        const params = this.parseEngineParameters(step);
        return await engine.generateText(params);
    }
    parseEngineParameters(step) {
        return {
    ...
    createEngineFromData(engineData) {
        if (engineData.name.includes('OpenAI') || engineData.id.includes('openai')) {
            return new OpenAIEngine(undefined, engineData.endpoint);
        }
        else if (engineData.name.includes('Anthropic') || engineData.id.includes('anthropic')) {
            return new ClaudeEngine(undefined, engineData.endpoint);
        }
        else {
            return new OpenAIEngine(undefined, engineData.endpoint);
        }
    }
    async recordExecutionToDatabase(workflowId, userId) {
        try {
            return await databaseManager.recordWorkflowExecution({
            ...
            console.error('记录工作流执行失败:', error);
            throw new Error(`工作流执行记录失败: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    async updateExecutionInDatabase(executionId, status, result, errorMessage) {
        try {
            await databaseManager.updateWorkflowExecution(executionId, status, result, errorMessage);
        }
        catch (error) {
            console.error('更新执行状态失败:', error);
            throw new Error(`执行状态更新失败: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    async recordStepExecutionToDatabase(executionId, stepId, status, inputData) {
        return await databaseManager.recordStepExecution({
    ...
    async updateStepExecutionInDatabase(historyId, status, outputData, errorMessage) {
        await databaseManager.updateStepExecution(historyId, status, outputData, errorMessage);
    }
}
