...
export class AuthService {
    ...
    jwtSecret;
    constructor() {
        this.databaseService = PostgresDatabaseService.getInstance();
    ...
    async register(userData) {
        ...
        const existingUser = await this.databaseService.getUserByEmail?.(validatedData.email);
        if (existingUser) {
            throw new Error('User already exists');
        }
        const hashedPassword = await this.hashPassword(validatedData.password);
        const user = await this.databaseService.createUser({
            username: validatedData.name || validatedData.email,
