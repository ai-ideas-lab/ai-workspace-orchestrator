...
const createTaskSchema = z.object({
    type: z.enum(['text-generation', 'code-analysis', 'image-generation', 'document-processing']),
...
router.post('/task', (req, res) => {
    try {
        ...
        const newTask = aiEngine.executeTask({
...
router.get('/task/:taskId', (req, res) => {
    try {
        ...
        const task = aiEngine.getTaskStatus(taskId);
        if (!task) {
            return res.status(404).json({
                success: false,
                error: '任务不存在'
...
router.delete('/task/:taskId', (req, res) => {
    try {
        const { taskId } = req.params;
        const cancelled = aiEngine.cancelTask(taskId);
        if (!cancelled) {
            return res.status(404).json({
                success: false,
                error: '任务不存在或无法取消'
...
router.get('/queue', (req, res) => {
    try {
        const queueStatus = aiEngine.getQueueStatus();
        res.json({
            success: true,
            data: queueStatus
        });
...
router.post('/cleanup', (req, res) => {
    try {
        ...
        aiEngine.cleanupCompletedTasks(maxAgeNumber);
        res.json({
            success: true,
            message: '已完成任务清理完成'
        });
...
router.post('/test-engine', async (req, res) => {
    try {
        ...
        const testTask = aiEngine.executeTask({
        ...
        while (Date.now() - startTime < maxWaitTime) {
            const currentTask = aiEngine.getTaskStatus(testTask.id);
        ...
        res.status(408).json({
            success: false,
            error: '测试超时',
            data: aiEngine.getTaskStatus(testTask.id)
        });
    }
    catch (error) {
        res.status(500).json({
