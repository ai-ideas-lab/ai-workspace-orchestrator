...
export class AIEngineConfigManager {
    ...
    saveConfiguration() {
        try {
            ...
            const configDir = path.dirname(this.storagePath);
            if (!fs.existsSync(configDir)) {
                fs.mkdirSync(configDir, { recursive: true });
            }
            fs.writeFileSync(this.storagePath, JSON.stringify(config, null, 2));
        }
        catch (error) {
    ...
    loadConfiguration() {
        try {
            if (fs.existsSync(this.storagePath)) {
                const configData = fs.readFileSync(this.storagePath, 'utf8');
...
export const aiEngineConfigManager = new AIEngineConfigManager();
