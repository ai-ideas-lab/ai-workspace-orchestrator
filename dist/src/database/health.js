...
export class DatabaseHealth {
    ...
    static async checkForeignKeys() {
        try {
            const violations = [];
            const workflowsWithInvalidUser = await prisma.workflow.findMany({
                where: {
                    user: {
                        is: {
                            id: null,
            ...
            const executionsWithInvalidWorkflow = await prisma.workflowExecution.findMany({
                where: {
                    workflow: {
                        is: {
                            id: null,
            ...
            const agentExecutionsWithInvalidAgent = await prisma.agentExecution.findMany({
                where: {
                    agent: {
                        is: {
                            id: null,
    ...
    static async checkDataIntegrity() {
        try {
            ...
            const nullConstraints = await prisma.user.count({
                where: {
                    OR: [
                        { email: null },
                        { username: null },
      ...
      `;
            ...
            const executionsWithoutAgents = await prisma.workflowExecution.count({
                where: {
                    agents: {
                        none: {},
                    },
            ...
            const agentsWithoutExecutions = await prisma.agent.count({
                where: {
                    executions: {
                        none: {},
                    },
            ...
            const workflowsWithInvalidConfig = await prisma.workflow.count({
                where: {
                    config: {
                        not: {},
                    },
