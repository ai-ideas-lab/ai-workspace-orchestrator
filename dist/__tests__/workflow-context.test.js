...
describe('WorkflowContext', () => {
    describe('步骤输出测试', () => {
        it('setStepOutput / getStepOutput — 存取步骤输出', () => {
            const ctx = new workflow_context_1.WorkflowContext('wf-1');
            ctx.setStepOutput('step-a', { score: 95, label: 'good' });
            const output = ctx.getStepOutput('step-a');
        ...
        test('resolveObject — 递归解析对象中所有表达式', () => {
            ...
            assertEqual(resolved.message, 'Found 2 users');
            assertDeepEqual(resolved.data, ['Alice', 'Bob']);
            assertEqual(resolved.extra.info, '2 items');
        });
        ...
        test('变更记录包含时间戳和 workflowId', () => {
            const ctx = new workflow_context_1.WorkflowContext('wf-test-123');
            ctx.setVariable('k', 'v');
            const log = ctx.getChangeLog();
        ...
        it('workflowId 存储正确', () => {
            const ctx = new workflow_context_1.WorkflowContext('my-workflow');
            expect(ctx.workflowId).toBe('my-workflow');
        });
    });
});
//# sourceMappingURL=workflow-context.test.js.map