...
describe('executeWorkflow', () => {
    ...
    describe('成功场景', () => {
        it('应该成功执行包含多个步骤的工作流', async () => {
            ...
            const mockEngine = {
                execute: globals_1.jest.fn().mockResolvedValue('Step completed successfully')
            };
            mockPrisma.workflow.findUnique.mockResolvedValue(mockWorkflow);
            const getEngine = globals_1.jest.fn().mockReturnValue(mockEngine);
            globals_1.jest.doMock('../src/core/executor', () => ({
                ...globals_1.jest.requireActual('../src/core/executor'),
                getEngine: getEngine
            }));
            const result = await (0, executor_1.executeWorkflow)('test-workflow-id', 'Test user input');
        ...
        it('应该处理单个步骤的工作流', async () => {
            ...
            const mockEngine = {
                execute: globals_1.jest.fn().mockResolvedValue('Single step completed')
            };
            mockPrisma.workflow.findUnique.mockResolvedValue(mockWorkflow);
            const getEngine = globals_1.jest.fn().mockReturnValue(mockEngine);
            globals_1.jest.doMock('../src/core/executor', () => ({
            ...
            const result = await (0, executor_1.executeWorkflow)('single-step-workflow', 'Test input');
    ...
    describe('错误处理场景', () => {
        it('应该处理工作流不存在的情况', async () => {
            ...
            await expect((0, executor_1.executeWorkflow)('non-existent-workflow', 'Test input'))
        ...
        it('应该处理步骤执行失败的情况', async () => {
            ...
            const result = await (0, executor_1.executeWorkflow)('workflow-with-failure', 'Test input');
        ...
        it('应该处理工作流获取过程中的数据库错误', async () => {
            ...
            await expect((0, executor_1.executeWorkflow)('workflow-id', 'Test input'))
    ...
    describe('边界条件测试', () => {
        it('应该处理空步骤列表的工作流', async () => {
            ...
            mockPrisma.workflow.findUnique.mockResolvedValue(mockWorkflow);
            const result = await (0, executor_1.executeWorkflow)('empty-workflow', 'Test input');
