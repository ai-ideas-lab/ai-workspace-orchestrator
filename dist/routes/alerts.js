...
const alerts_js_1 = require("../services/alerts.js");
const router = (0, express_1.Router)();
router.get('/', (req, res) => {
    try {
        const { status } = req.query;
        const alerts = alerts_js_1.alertService.getAlerts(status);
        res.json({
            success: true,
            data: alerts,
            count: alerts.length,
...
router.get('/summary', (req, res) => {
    try {
        const summary = alerts_js_1.alertService.getAlertSummary();
        res.json({
            success: true,
            data: summary,
            timestamp: new Date().toISOString()
...
router.post('/:id/resolve', (req, res) => {
    try {
        const { id } = req.params;
        const success = alerts_js_1.alertService.resolveAlert(id);
        if (success) {
            res.json({
                success: true,
...
router.post('/resolve-all', (req, res) => {
    try {
        const resolvedCount = alerts_js_1.alertService.resolveAllAlerts();
        res.json({
            success: true,
...
router.get('/rules', (req, res) => {
    try {
        const rules = alerts_js_1.alertService.getRules();
        res.json({
            success: true,
            data: rules,
            count: rules.length,
...
router.post('/rules', (req, res) => {
    try {
        ...
        const newRule = alerts_js_1.alertService.addRule({
            name,
            description,
            metric: metric,
            threshold,
...
router.put('/rules/:id', (req, res) => {
    try {
        ...
        const success = alerts_js_1.alertService.updateRule(id, updates);
        if (success) {
            res.json({
                success: true,
...
router.delete('/rules/:id', (req, res) => {
    try {
        const { id } = req.params;
        const success = alerts_js_1.alertService.deleteRule(id);
        if (success) {
            res.json({
                success: true,
...
router.delete('/history', (req, res) => {
    try {
        ...
        alerts_js_1.alertService.clearAlertHistory(keepRecent ? parseInt(keepRecent) : undefined);
        res.json({
            success: true,
...
router.get('/health', (req, res) => {
    try {
        const health = alerts_js_1.alertService.getSystemHealth();
        res.json({
            success: true,
            data: {
                health,
...
router.post('/monitor', (req, res) => {
    try {
        ...
        alerts_js_1.alertService.monitorSystemMetrics(mockMetrics);
        res.json({
            success: true,
